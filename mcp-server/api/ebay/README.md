# eBay Browse API and Taxonomy API Integration

This module provides integration with eBay's Browse API and Taxonomy API, allowing you to search for and discover items on eBay, as well as explore category structures and item aspects through the MCP server.
...
### Taxonomy API
- **Category Suggestions**: Get suggested categories based on keywords
- **Item Aspects**: Get required and recommended item specifics (aspects) for categories
...
### Tool: `get_category_suggestions`

Get suggested eBay categories based on keywords. This is particularly useful when listing items and you need to find the most appropriate category.
...
3. Returns suggested categories with their full hierarchy paths
...
#### Response
Returns:
- **categorySuggestions**: Array of suggested categories, each containing:
  - **category**: Category ID and name
  - **categoryTreeNodeLevel**: Depth level in the category tree
  - **categoryTreeNodeAncestors**: Full path of parent categories up to root
...
Get item aspects (item specifics) for a specific category. These are the required or recommended attributes for listing items (e.g., Brand, Color, Size).
...
#### Response
Returns:
- Category ID and tree information
- List of aspects with:
  - Aspect names (e.g., "Brand", "Color", "Size")
  - Aspect constraints (REQUIRED, RECOMMENDED, etc.)
