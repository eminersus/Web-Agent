...
class ChatService:
    ...
    async def stream_chat_with_tools(
        ...
        while iteration < max_iterations:
                    ...
                    for tool_call_chunk in chunk["tool_calls"]:
                        ...
                        if "function" in tool_call_chunk:
                            ...
                            if "name" in func_chunk:
                                tool_call_accumulator[index]["function"]["name"] = func_chunk["name"]
                            if "arguments" in func_chunk:
                                tool_call_accumulator[index]["function"]["arguments"] += func_chunk["arguments"]
            ...
            if tool_call_accumulator:
                tool_calls = [tool_call_accumulator[i] for i in sorted(tool_call_accumulator.keys())]
            ...
            if tool_calls or finish_reason == "tool_calls":
                ...
                for tool_call in tool_calls:
                    ...
                    try:
                        ...
                        tool_call_event = self._format_sse("tool_call", {
                            "id": tool_id,
                            "name": tool_name,
                            "arguments": tool_args
                        })
                        ...
                        tool_result_event = self._format_sse("tool_result", {
                            "id": tool_id,
                            "name": tool_name,
                            "result": result
                        })
                        logger.info(f"Sending tool_result event: {tool_result_event[:100]}")
            ...
            try:
                ...
                if isinstance(result_data, dict):
                    if "error" in result_data:
                        ...
                        response_text = f"Based on the {tool_name} tool: {result_data['result']}"
                    else:
                ...
                yield self._format_sse("token", {"token": f"Tool result: {result_content}"})
        else:
    ...
    def _convert_tools_to_openai_format(self, mcp_tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for tool in mcp_tools:
            try:
                ...
                logger.debug(f"Tool {tool.get('name')}: {json.dumps(openai_tool, indent=2)}")
            except Exception as e:
